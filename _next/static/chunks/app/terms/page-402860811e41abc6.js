(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
  [35571, 385, 80171],
  {
    25416: function (A, e, t) {
      Promise.resolve().then(t.t.bind(t, 85014, 23)),
        Promise.resolve().then(t.bind(t, 32543)),
        Promise.resolve().then(t.bind(t, 74493)),
        Promise.resolve().then(t.bind(t, 54467)),
        Promise.resolve().then(t.t.bind(t, 64845, 23));
    },
    64845: function () {},
    32543: function (A, e, t) {
      "use strict";
      t.r(e),
        (e.default = {
          src: "/_next/static/media/3dlogo.31629924.png",
          height: 1100,
          width: 1084,
          blurDataURL:
            "data:image/png;base64,",
          blurWidth: 8,
          blurHeight: 8,
        });
    },
  },
  function (A) {
    A.O(
      0,
      [
        64654, 96310, 54214, 20049, 90600, 17714, 24883, 63542, 48312, 54467,
        59062, 34034, 1744,
      ],
      function () {
        return A((A.s = 25416));
      }
    ),
      (_N_E = A.O());
  },
]);
